...
Define thread-safety before release. At minimum, separate model handles must be usable independently; document whether calls on one handle are serialized or concurrent. Bound or expose worker-thread configuration so the component can coexist with its host application. Make resource release explicit and test repeated load/unload cycles.
