...
Status: experimental component v0.2, 2026-09-17. Native builds, Python, Java, Node.js/TypeScript, browser WebAssembly, open training data, and the training pipeline are implemented. Platform qualification is recorded in the [release inventory](../released/README.md) and test reports. The decision-quality requirements below remain targets; the current trained release has not met them. Start with [the current recipe](accuracy-v2.md) and [results](../reports/accuracy-v2.md). The [first-version recipe](first-version.md) remains available for reproducing the original experiment.
