...
const source = resolve(process.env.DECISIONGATE_BUNDLE || join(root, '..', 'released', sourceName));
...
const addon = join(output, 'decisiongate.node');
if (process.platform === 'win32') {
  ...
  execFileSync('cl', ['/nologo', '/std:c++17', '/EHsc', '/LD', `/Fo${join(buildDirectory, 'addon.obj')}`,  `/I${headers}`, join(root, 'src/addon.cc'), lib, '/link', `/IMPLIB:${join(buildDirectory, 'decisiongate-addon.lib')}`, `/OUT:${addon}`], { cwd: root, stdio: 'inherit' });
} else {
