...
# ///
"""Score Laya and DecisionGator on the same held-out tests, on this machine's CPU.

  uv run code/compare_laya.py [OUTPUT_DIR]

Runs each Laya checkpoint and the DecisionGator release for this platform over the
...
import decisiongator  # noqa: E402
...
def run_decisiongator(session, record):
    if record['label_type'] == 'choice':
...
def main():
    ...
    session = decisiongator.Session.load(decisiongator._bundled_directory())
    results = {'decisiongator': score('decisiongator', lambda r: run_decisiongator(session, r))}
    session.close()
