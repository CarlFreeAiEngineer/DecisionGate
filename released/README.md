...
**Version status by platform.** All three native bundles (Mac, Windows x64, Linux x64), their Python wheels, Node tarballs, and Java classifier JARs, and the browser assets are version 0.4.0: the DeBERTa-v3-large model, stored as float16 weights. Windows and Linux were rebuilt on 19 September 2026 from the same exported model as the Mac bundle; the frozen cross-platform comparison cases agree on every boolean decision, with probability differences under one millionth.
...
The 0.4.0 browser bundle is approximately 870 MB and each 0.4.0 native bundle approximately 870 to 900 MB (weights 872 MB, stored as float16 and computed in float32). A running process holds about 5 GB, because the runtime keeps a float32 copy of the weights. The previous 0.3.0 Mac bundle (229 MB) is preserved at `models/macos-arm64-v3/` for anyone who prefers size over accuracy. The combined npm tarball contains separate native and browser assets, including two copies of the weights, so it is larger. Browser use requires the web app's initial asset delivery and sufficient storage for offline installation. See [browser measurements and limitations](../reports/browser-webassembly.md).
