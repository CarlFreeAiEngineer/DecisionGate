...
- **Laya** is the open answer to Jev: a 421-million-parameter encoder (a model that reads the whole text at once instead of writing words) with typed choice, score, and yes/no outputs and calibrated probabilities, Apache-2.0 weights, and about 35 milliseconds per question on a server GPU. Unlike Jev, it runs on your own machine. The difference is packaging: Laya is a Python package on PyTorch (plus a TypeScript port) that downloads its model from Hugging Face on first use, while DecisionGate is one library file with ready-made interfaces for seven languages and a browser build. Laya asks bigger questions (choices and scores, not just yes/no) and is bigger to carry.
