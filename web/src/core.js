...
export class DecisionGatorError extends Error {
  constructor(code, message) { super(message); this.name = 'DecisionGatorError'; this.code = code; }
}
...
export function text(value, name) {
  if (typeof value !== 'string') throw new DecisionGatorError('DG_INVALID_ARGUMENT', `${name} must be nonempty text`);
  let bytes = 0;
  for (let i = 0; i < value.length; ++i) {
    ...
    if (c >= 0xd800 && c <= 0xdbff) {
      ...
      if (!(next >= 0xdc00 && next <= 0xdfff)) throw new DecisionGatorError('DG_INVALID_ARGUMENT', `${name} contains an unmatched surrogate`);
    } else if (c >= 0xdc00 && c <= 0xdfff) throw new DecisionGatorError('DG_INVALID_ARGUMENT', `${name} contains an unmatched surrogate`);
    if (bytes > 1_048_576) throw new DecisionGatorError('DG_INPUT_TOO_LONG', `${name} exceeds the byte limit`);
  }
  if (blank.test(value)) throw new DecisionGatorError('DG_INVALID_ARGUMENT', `${name} must be nonempty text`);
}
export function validate(content, question, options = {}, boolean = false, defaultThreshold = 0.5) {
  ...
  if (options.criteria !== undefined && options.criteria !== null) {
    if (typeof options.criteria !== 'object' || Array.isArray(options.criteria)) throw new DecisionGatorError('DG_INVALID_ARGUMENT', 'criteria must contain yes and no text');
  ...
  if (boolean && (typeof threshold !== 'number' || !Number.isFinite(threshold) || threshold < 0 || threshold > 1)) throw new DecisionGatorError('DG_INVALID_ARGUMENT', 'threshold must be finite and within [0, 1]');
  return threshold;
}
const MIN_OPTIONS = 2;
...
export function validateOptions(options) {
  if (!Array.isArray(options) || options.length < MIN_OPTIONS || options.length > MAX_OPTIONS) throw new DecisionGatorError('DG_INVALID_ARGUMENT', `options must be an array of ${MIN_OPTIONS} to ${MAX_OPTIONS} strings`);
...
export function rank(logits) {
  ...
  if (ranking.some(({ p }) => !Number.isFinite(p) || p < 0 || p > 1)) throw new DecisionGatorError('DG_INFERENCE_ERROR', 'Invalid choice probability');
...
export function createTokenizer(json) {
  if (!["RobertaProcessing", "TemplateProcessing"].includes(json.post_processor?.type)) throw new DecisionGatorError("DG_INCOMPATIBLE", "This release requires a RoBERTa or template (DeBERTa) tokenizer");
...
export function encode(tokenizer, manifest, content, question, criteria) {
  ...
  if (encoded.ids.length > manifest.max_tokens) throw new DecisionGatorError('DG_INPUT_TOO_LONG', 'Input exceeds the token limit; input was not truncated');
