...
Serve this directory over HTTPS (or localhost for development). Preserve filenames and relative locations. Serve `.js` and `.mjs` as JavaScript and `.wasm` as `application/wasm`. The component uses CPU WebAssembly in a dedicated module worker; no GPU setup is needed. It runs on one thread unless the page is cross-origin isolated, which lets it use every core and makes calls about two to five times faster. To enable that, serve the page and its assets with `Cross-Origin-Opener-Policy: same-origin` and `Cross-Origin-Embedder-Policy: require-corp`; resources from other origins then need their own permission headers. Assets are about 600 MB uncompressed, including 574 MB of weights in four `weights-N.bin` files beside a small `model.onnx` graph. Keeping the weights in separate files lets the runtime use them without copying, which roughly halves memory while loading. Startup and memory still depend on the device.
...
Chromium, Firefox, and Playwright WebKit passed 691 exact tokenization cases, 87 native probability comparisons, and cached offline reload followed by inference on this Apple silicon Mac, with and without cross-origin isolation. The same runs also compared 5 `chooseP` rankings to native `choose` results, including a tied pair of options and a threshold that always returns -1; maximum probability difference was 0.021. With version 0.4.1's quantized weights, browser answers differ from native ones by up to 0.088 in probability, and 2 of the 87 comparison decisions came out differently, because WebAssembly rounds 8-bit arithmetic differently; the tests allow the tolerance recorded in `manifest.json`. Typical warm calls took about 150 to 400 ms on a cross-origin isolated page and about 680 to 760 ms without isolation, against about 80 ms for the much smaller 0.3.0 model. These measurements cover desktop browsers; mobile devices and the installed Safari application have not been tested. The full evidence is in the project’s `reports/browser-webassembly.md`.
