...
// DECISIONGATOR_NATIVE_DIR stages a different native bundle (its manifest, weights, tokenizer, and notices).
const native = path.resolve(process.env.DECISIONGATOR_NATIVE_DIR ?? path.join(root, 'released/macos-arm64'));
const manifest = JSON.parse(await readFile(path.join(native, 'manifest.json')));
// The browser gets the model as a small graph plus a few weight files (see split_model.py).
const weights = JSON.parse(execFileSync('uv', ['run', path.join(here, 'split_model.py'), path.join(native, 'model.onnx'), destination], { encoding: 'utf8' }));
manifest.sha256 = { 'tokenizer.json': manifest.sha256['tokenizer.json'] };
for (const file of ['model.onnx', ...weights]) manifest.sha256[file] = createHash('sha256').update(await readFile(path.join(destination, file))).digest('hex');
manifest.weights = weights.map(file => ({ file, bytes: statSync(path.join(destination, file)).size }));
...
for(const file of ['tokenizer.json']) await copyFile(path.join(native,file),path.join(destination,file));
await copyFile(path.join(here,'src/index.d.ts'),path.join(destination,'index.d.ts'));
await writeFile(path.join(destination,'package.json'),JSON.stringify({name:'decisiongator-web',version:'0.4.1',private:true,type:'module',exports:{'.':{types:'./index.d.ts',import:'./index.js'}}},null,2)+'\n');
